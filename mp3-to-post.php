...
function mp3_admin_actions() {
  add_options_page("MP3 to Post", "MP3 to Post", 1, "mp3-to-post", "mp3_admin");
}
...
function mp3_admin() {
  ...
  <div class="wrap">
    ...
    <form method="post" action="">
      <input type="submit" class="button-primary" name="create-all-posts" value="<?php _e('Create All Posts') ?>" />
      <input type="submit" class="button-primary" name="create-first-post" value="<?php _e('Create 1st Post') ?>" />
    </form>
    <?php      
...
function mp3_to_post($limit = 'all', $folderPath) {
  ...
  if(count($mp3Files) == 0){
    array_push($messages, 'There are no files to process');
    return $messages;
  }
  ...
  while ($i <= $limit):
    ...
    if ($title && $comment){
      ...
      if ($titleSearchResult->post_count == 0) {
        ...
        // 
        array_push($messages, 'Post created: ' . $title);
      } else {
